...
export class HUDScene extends Phaser.Scene {
    ...
    create(): void {
        this.textElements = new Map([
            [
                "LIVES",
                this.addText(0, 0, `MARIOx ${this.registry.get("lives")}`)
            ],
            [
                "WORLDTIME",
                this.addText(80, 0, `${this.registry.get("worldTime")}`)
            ],
        ...
        level.events.on("timeChanged", this.updateTime, this);
    ...
    private updateLives() {
        this.textElements
            .get("LIVES")
            ?.setText(`MARIOx ${this.registry.get("lives")}`);
    }
